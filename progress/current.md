...
- 下一步：进入 AI API 工程，开始实现具备输入契约、超时、重试和错误分类的可靠模型 API 调用器
- 状态：数据思维收尾已完成；天气、用户类型、更多图表和完整 EDA 报告不再作为必修
...
1. 实现最小可运行的模型 API 调用器，理解业务请求、输入契约、HTTP 调用与结构化结果之间的数据流。
2. 加入环境变量、超时、重试和错误分类，区分可重试故障与不可重试故障。
3. 使用类型、Pydantic 和 pytest 验证正常响应、异常响应与失败边界。
