...
- 周次：第 4 周进行中
- 已学习：第 4 周，第 4 课主体，探索性数据分析报告与统计直觉（2026-08-25）
- 待完成：第 4 周第 4 课复盘小测、Notebook 最终检查与课程验收
- 状态：已完成描述性统计、缺失追踪、分组 IQR、相关性分析和四类核心图表实践
- 项目：`projects/week-04-data/eda_report.ipynb`（第四课主体实践完成）
- 当前学习阶段：能够把样本量、分布、异常规则、相关性与图表组织成初步 EDA 报告

## 下一步

1. 回答第 4 周第 4 课复盘小测，并完成 Notebook 最终检查。
2. 重启内核并从头运行 `eda_report.ipynb`，确认单元格顺序、输出和管理摘要可复现。
3. 完成第四周课程验收后，进入第 5 周真实公开数据分析。

## 第 4 周 · 第 4 课阶段成果（主体完成，待验收）
