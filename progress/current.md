...
- 周次：第 5 周进行中
- 已完成实践：第 5 周，第 2 课，系统化数据清洗与可追踪转换（2026-08-29；复盘小测待完成）
- 下一步：完成第 5 周第 2 课复盘小测，再进入第 5 周第 3 课
- 状态：严格类型转换、类别标签映射、业务尺度恢复、缺失小时需求时间轴、高值标记和导出回读验证已完成
- 项目：`projects/week-05-bike-sharing/notebooks/02_data_cleaning.ipynb`（47 个代码单元格全部执行，0 个错误输出）
- 当前学习阶段：已建立可复现、可追踪的数据清洗函数与质量闸口，准备完成复盘并进入分组比较和可视化

## 下一步

1. 完成第 5 周第 2 课复盘小测，巩固审计、清洗、记录缺失和字段缺失的区别。
2. 进入第 5 周第 3 课，基于清洗结果比较小时、工作日、季节、天气和用户类型的需求模式。
3. 继续巩固目标泄漏与时间切分：预测 `cnt` 时排除 `casual`、`registered`，并用过去预测未来。
