...
- 路线：6 周 AI 应用工程核心路线（2026-09-01 起）
- 当前阶段：第 1 周，数据思维收尾与 AI API 工程入口
- 下一步：创建独立的 `model-api-client` 项目，用 pytest 从输入契约和配置验证开始实现
- 状态：数据思维收尾与可靠模型 API 调用器概念部分已完成，下一阶段进入代码实践
- 项目：`projects/week-05-bike-sharing/notebooks/03_group_comparison.ipynb`（32 个代码单元格全部执行，0 个错误输出）
- 当前学习阶段：从通用 Python/数据/机器学习路线转向银行 AI 应用原型；代码继续提供，但加强问题、数据流、API 选择、失败边界和验证方式的解释

## 下一步

1. 创建 `projects/model-api-client/` 的最小 Python 工程和测试环境。
2. 用 pytest 驱动实现 `ModelRequest`、`ModelResult` 与 `ApiConfig`。
3. 实现带显式超时、错误分类和有限重试的模拟 HTTP 调用，并覆盖失败边界。

## AI 应用工程核心路线 · 第 1 周第 1 课概念成果

- 理解模型 API 调用的数据流：业务请求经过输入校验、配置读取、HTTP 调用、状态检查和 JSON 契约验证后，才成为业务结果。
- 能区分本地边界与远程边界：空输入和缺失配置可在本地发现；过期密钥、限流、服务故障及响应异常通常在远程调用后发现。
- 理解 `dataclass(frozen=True)`、`__post_init__()` 和 `object.__setattr__()` 在不可变请求对象规范化与校验中的职责。
- 理解 `@classmethod` 提供命名构造方式；只有显式调用 `ApiConfig.from_environment()` 才会读取环境变量。
- 能按恢复可能性分类错误：输入、鉴权和响应契约错误默认不重试；`429`、网络异常和部分 `5xx` 只进行有限重试。
- 理解显式超时保护用户等待时间、连接和应用处理资源，而不只是一个性能参数。
- 理解 `lambda: client.ask(request)` 是延迟执行的零参数函数，类似 Java `Supplier<ModelResult>`，使重试器能够控制调用次数。
- 能区分 `max_attempts` 与 `max_retries`：本课 `max_attempts=3` 包含首次调用，总共最多发送 3 次请求。
- 理解依赖注入与 `httpx.MockTransport` 的连接：模型客户端使用注入的 HTTP 客户端，测试传输把请求交给本地 `handler`，无需访问真实服务。
- 能区分合法 JSON 与合法业务响应，并验证顶层对象、`answer` 和 `model` 的类型及非空约束。
- 下一步不再增加概念范围，进入独立项目实践；具体模型厂商 API 和 Pydantic 结构化输出留到后续课程。

## 新路线决策

- 职业定位：银行 AI 平台项目经理，需要具备 AI 应用工程理解和小型原型验证能力。
- 时间范围：6 周，每周约 8–10 小时。
- 主项目优先级：银行制度或产品文档 RAG；模型网关、工具调用和通用助手作为扩展方向。
- 核心内容：最小 Python 工程、输入契约、LLM API、结构化输出、RAG、评估、安全治理、FastAPI、可观测性和 Docker。
- 降级内容：大量 Python 数据 API、传统算法全景、复杂统计、调参、深入 asyncio 和 Agent 框架大全。
- 教学方式：提供完整可运行代码；在代码前解释业务问题和预期结构，在代码后拆解数据流、语法、取舍、失败边界和验证方式。
- 学习验收：能够解释和修改代码、运行测试、诊断失败并 Review 方案，不要求从空白默写大量 API。
- 数据边界：只使用公开或合成银行材料，不接触真实客户信息或未经批准的内部数据。

## 第 5 周 · 数据思维收尾成果（已完成）

- 创建 `03_group_comparison.ipynb`，从 `hourly_clean.csv` 独立读取 17,379 行、26 列数据并通过输入契约。
- 使用中性 `season_1` 至 `season_4` 展示名规避季节数据字典版本冲突，并验证日期与原始星期编码一致。
- 按小时生成 24 行汇总表，计算样本量、总租赁量、均值、中位数和 P95，验证记录数与租赁总量守恒。
- 按日类型与小时生成 48 行汇总表；工作日中位需求在 17 点最高（539，样本量 499），非工作日 12、13 点并列最高（367，样本量均为 231）。
- 绘制工作日与非工作日小时需求及样本量曲线，识别工作日早晚双峰和非工作日午后宽峰，同时明确模式与通勤解释一致但不能证明出行目的。
- 创建 7 × 24 的星期 × 小时中位需求矩阵和样本量矩阵，验证样本量总和为 17,379。
- 绘制星期 × 小时热力图；发现工作日早晚高峰、周末午后宽峰，并确认每格样本量为 92–105，最低样本主要位于凌晨。
- 完成年份 × 季节编码汇总；四个季节组在 2012 年的平均需求均高于 2011 年，同时明确描述性差异不能直接证明增长原因。
- 明确预测时点是特征可用性的判断基准：`casual`、`registered` 和由目标生成的 `cnt_iqr_high` 构成目标泄漏；工作日可由日历提前得知，实际天气与天气预报必须区分。
- 建立特征审计表，并通过 `cnt == casual + registered` 的关系断言验证直接目标泄漏。
- 按真实时间顺序切分训练集、验证集和保留测试集：2011 年训练 8,645 行，2012 年上半年验证 4,358 行，2012 年下半年保留 4,376 行；验证时间边界互不交叉且记录总数守恒。
- 理解训练集用于学习、验证集用于反复比较方案、测试集用于选定方案后的最终评估；反复依据验证结果调整方案会产生对验证集的间接过拟合。
- 建立训练集总体中位数基线：统一预测 109，验证集 MAE 为 159.95。
- 建立“工作日 × 小时”历史中位数查找基线：验证集 MAE 为 86.12，相比总体中位数基线降低约 46.2%。
- 理解 MAE 表示平均绝对数量偏差，不是准确率；复杂模型必须超过简单、低成本、可解释的基线，才具备可讨论的增量价值。
- 将数据泄漏思维迁移到 RAG 评测：正式业务文档属于允许检索的证据，评测标准答案、测试题定制材料和业务时点之后才出现的文档属于泄漏风险。
- 当前 Notebook 有 32 个代码单元格，全部执行且无错误输出；天气、用户类型、更多敏感性图表、资产导出和完整小测转为选修。

## 第 5 周 · 第 2 课成果

- 创建并从头执行 `02_data_cleaning.ipynb`，将数据审计结果转化为有依据、可验证的清洗策略。
- 建立原始输入基线，确认 17,379 行、17 个原始字段、0 个完整重复行、0 个缺失单元格和总租赁量 3,292,679。
- 使用严格日期格式和 `errors="raise"` 转换 `dteday`，创建唯一、升序的小时级 `timestamp`。
- 保留 `season`、`weathersit` 和 `weekday` 原始编码，并新增完整映射的可读类别标签，标签列在内存中使用 `category` 类型。
- 依据官方恢复系数新增摄氏温度、摄氏体感温度、湿度百分比和风速缩放字段，不擅自猜测官方未明确的风速单位。
- 区分字段缺失和记录缺失，识别完整时间轴的 165 个缺失小时，并仅在独立需求时间轴中对有证据支持的三个租赁计数字段补 0。
- 验证完整需求时间轴共 17,544 行、165 个缺失记录标记、计数字段无缺失，并保持总租赁量和 `cnt == casual + registered` 关系不变。
- 确认没有完整重复行和 `dteday + hr` 业务键重复行，理解删除重复前必须先判断数据产生原因。
- 使用全局 IQR 上界 642.5 标记 505 条高需求候选；其中 81% 集中在 8、17、18 点，因此保留真实通勤高峰而不机械删除。
- 编写 `map_required()` 与 `clean_hourly_data()`，验证函数不修改输入对象，并在未知季节编码 5 出现时抛出明确错误。
- 新增 9 个可追踪字段，输出表保持 17,379 行、26 列，并通过日期、键唯一性、类别映射、关系契约和总量守恒检查。
- 生成转换日志，记录每一步的理由、前后行数和影响行数；所有主表转换均未改变行数。
- 导出 `data/processed/hourly_clean.csv`，重新读取后验证形状、日期、时间键、关系契约、总租赁量和高值标记全部一致。
- 理解 CSV 不保存 Pandas `category` 元数据，回读时标签列会恢复为 `object`，需要显式转换回类别类型。
- Notebook 验收结果：47 个代码单元格全部执行，0 个未执行代码单元格，0 个错误输出。
- 原第 5 周第 2 课完整复盘小测转为选修；与输入契约、错误处理和数据泄漏相关的知识会在新路线中继续复用。

## 第 5 周 · 第 1 课成果

- 选择并获取 UCI Bike Sharing Dataset，记录官方来源、采集背景、时间范围、文件粒度和字段定义。
- 建立 `projects/week-05-bike-sharing/` 项目结构，区分不可手工修改的 `data/raw`、后续清洗使用的 `data/processed` 和分析 Notebook。
- 明确 `hour.csv` 的观察单位是一个至少发生一次租赁的小时汇总记录，`day.csv` 的观察单位是一个自然日。
- 创建 `01_data_audit.ipynb`，读取 17,379 行、17 个原始字段，并将 `dteday` 从 `object` 转换为 `datetime64[ns]`。
- 创建小时级 `timestamp`，验证时间戳唯一且升序，同时理解唯一和有序不能保证时间序列连续。
- 识别 75 处时间断点、165 个缺失小时和 76 个不完整日期，并发现缺失主要集中在凌晨 2–5 点。
- 将小时表按日聚合后与 731 行的 `day.csv` 一对一连接，验证 `casual`、`registered` 和 `cnt` 每日总量全部一致。
- 结合现有小时记录最小 `cnt` 为 1、零租赁记录数为 0 的证据，确认缺失小时对每日租赁总量的贡献为 0。
- 完成列级审计：原始字段无列内缺失、无完整重复行、无重复的 `dteday + hr` 业务键，`instant` 唯一。
- 为类别编码、归一化字段和非负计数字段建立范围契约，所有断言通过。
- 验证全部 17,379 行满足 `cnt == casual + registered`，理解单列范围检查不能替代跨字段关系检查。
- 分析 `cnt` 的均值、中位数、P95、P99 和最大值，并通过直方图识别右偏分布，理解偏度名称由长尾方向决定。
- 绘制全局小时需求曲线以及工作日与非工作日对比图，发现工作日早晚双峰和非工作日午后宽峰。
- 使用“观察 + 限制 + 下一步”记录图表结论，明确小时模式与通勤解释一致，但数据不能证明出行目的或因果关系。
- 建立项目 README，记录数据来源、目录约定、已确认的数据契约、已知限制、目标泄漏和时间切分风险。
- Notebook 验收结果：32 个代码单元格全部执行，0 个未执行代码单元格，0 个错误输出。
- 待巩固：整数存储类型与类别业务语义的区别；`casual`、`registered` 对预测 `cnt` 构成目标泄漏的原因；时间数据优先按时间切分的评估含义。
