...
- 周次：第 5 周进行中
- 当前课程：第 5 周第 3 课进行中（2026-09-01），已完成工作日小时曲线与星期 × 小时热力图
- 下一步：从年份 × 季节编码比较继续；第 2 课复盘小测仍待完成
- 状态：已完成分组汇总、日类型比较、样本量检查和星期 × 小时需求矩阵，季节、天气、用户类型、敏感性与导出阶段尚未完成
- 项目：`projects/week-05-bike-sharing/notebooks/03_group_comparison.ipynb`（20 个代码单元格全部执行，0 个错误输出）
- 当前学习阶段：从跟随式复现转向主动分析训练，后续先定义问题、粒度、分组列和指标，再独立编码并接受 Review

## 下一步

1. 继续第 5 周第 3 课：先自主设计年份 × 季节编码汇总，再编写代码。
2. 完成天气、用户类型和零需求小时敏感性分析，并导出第三课报告资产。
3. 补完第 2、3 课复盘小测；继续巩固目标泄漏与时间切分。
...
- 待完成：季节编码、天气、用户类型、零需求小时敏感性、资产导出、课堂练习和小测。
