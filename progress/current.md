...
- 周次：第 5 周进行中
- 已完成：第 5 周，第 1 课，真实公开数据集、数据来源与数据契约（2026-08-27）
- 下一课：第 5 周，第 2 课
- 状态：UCI 共享单车数据来源核验、时间完整性分析、数据契约和第一轮需求画像已完成
- 项目：`projects/week-05-bike-sharing/notebooks/01_data_audit.ipynb`（32 个代码单元格全部执行，0 个错误输出）
- 当前学习阶段：已从合成数据迁移到真实公开数据，准备进入系统化清洗、类别解释和可追踪转换

## 下一步

1. 进入第 5 周第 2 课，基于审计结果制定类型、缺失、类别和异常处理策略。
2. 巩固“存储类型不等于业务语义”：`season`、`weekday` 等整数编码属于类别变量。
3. 巩固目标泄漏与时间切分：预测 `cnt` 时排除 `casual`、`registered`，并用过去预测未来。
