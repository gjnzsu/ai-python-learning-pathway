...
- 下一步：完成一个年份 × 季节编码汇总练习，随后实现可靠的模型 API 调用器
- 状态：原第 5 周第 3 课选择性收尾；天气、用户类型、更多图表和完整 EDA 报告不再作为必修
- 项目：`projects/week-05-bike-sharing/notebooks/03_group_comparison.ipynb`（20 个代码单元格全部执行，0 个错误输出）
- 当前学习阶段：从通用 Python/数据/机器学习路线转向银行 AI 应用原型；代码继续提供，但加强问题、数据流、API 选择、失败边界和验证方式的解释

## 下一步

1. 使用完整参考代码完成年份 × 季节编码汇总，并解释分组粒度、样本量、均值、中位数和结论边界。
2. 从第五周第四课只学习数据泄漏、时间切分、简单基线和证据追踪，不制作完整 EDA 报告。
3. 进入模型 API：HTTP、JSON、环境变量、超时、重试、错误分类、类型和 pytest。
...
- 当前 Notebook 有 20 个代码单元格，全部执行且无错误输出。
- 学习方式调整：后续不再直接复制完整代码，先预测结果粒度和结构，自主选择分组列与指标，再按最小提示逐步实现。
- 核心收尾：只完成年份 × 季节编码练习；天气、用户类型、更多敏感性图表、资产导出和完整小测转为选修。
